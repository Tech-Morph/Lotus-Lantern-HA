"""Shared BLE connection manager for an ELK-BLEDDM device.
...
class ElkBleddmDevice:
    """Owns the single BLE connection to one ELK-BLEDDM device."""
    ...
    def _on_unexpected_disconnect(self, client: BleakClientWithServiceCache) -> None:
        _LOGGER.warning("ELK-BLEDDM %s disconnected unexpectedly", self.address)
        self._client = None
