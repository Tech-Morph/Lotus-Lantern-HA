"""Light platform for Lotus Lantern / ELK-BLEDDM strips.

Refactored to use the shared ElkBleddmDevice connection manager
(device.py) instead of owning its own BleakClient. Color writes check
device.music_reactive to decide between the normal color command and
the music-reactive color variant, so the "Music Reactive Color" switch
entity actually changes behavior.
"""
...
async def async_setup_entry(
    ...
    name: str = entry.data.get("name", "Lotus Lantern Light")
    async_add_entities([ElkBleddmLight(device, name)])


class ElkBleddmLight(LightEntity):
    """Representation of a Lotus Lantern / ELK-BLEDDM BLE light strip."""
